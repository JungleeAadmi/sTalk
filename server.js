// server.js (sTalk) - drop-in replacement aligned with final index.html & app.js
'use strict';
...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'database', 'stalk.db');
...
// Candidate locations for a .vapid.json installer file
...
// Attempt to load VAPID keys from file locations
function loadVapidFromFile() {
  for (const p of DEFAULT_VAPID_LOCATIONS) {
    try {
      ...
      console.warn('⚠️ Error reading VAPID file', p, ':', err && err.message ? err.message : err);
    }
  }
  return false;
}
...
// Security middleware (CSP disabled to allow service worker + inline for simple PWA)
app.use(helmet({
...
// Multer: dynamic storage destination by mimetype
const storage = multer.diskStorage({
  destination(req, file, cb) {
    let uploadDir = path.join(UPLOAD_PATH, 'files');
    if (file.mimetype && file.mimetype.startsWith('image/')) uploadDir = path.join(UPLOAD_PATH, 'images');
    else if (file.mimetype && file.mimetype.startsWith('audio/')) uploadDir = path.join(UPLOAD_PATH, 'audio');
    else if (file.mimetype && (file.mimetype.includes('pdf') || file.mimetype.includes('msword') || file.mimetype.includes('document'))) uploadDir = path.join(UPLOAD_PATH, 'documents');
    cb(null, uploadDir);
  },
...
// Profile image storage (writes to PROFILE_PATH)
...
app.post('/api/profile/image', authenticateToken, uploadLimiter, profileUpload.single('profileImage'), (req, res) => {
  ...
  db.get('SELECT profile_image FROM users WHERE id = ?', [req.user.id], (err, userRow) => {
    if (userRow && userRow.profile_image) {
      try {
        const oldImagePath = path.join(PROFILE_PATH, path.basename(userRow.profile_image));
    ...
    db.run('UPDATE users SET profile_image = ? WHERE id = ?', [publicPath, req.user.id], (err) => {
      ...
      res.json({ message: 'Profile image updated successfully', profileImage: publicPath });
    });
  });
});
...
// Get chat messages with other user
...
// Send message to other user (text or file)
  ...
  db.get('SELECT username, id FROM users WHERE id = ?', [otherUserId], (err, otherUser) => {
    ...
    db.run(`INSERT INTO messages (chat_id, sender, content, message_type, file_path, file_name, file_size, file_type) 
      ...
      db.get(`SELECT m.*, u.full_name, u.avatar, u.profile_image 
        ...
        // Send sockets to both sender (ack) and recipient (incoming)
...
app.get('/api/push/key', (req, res) => {
  if (!VAPID_PUBLIC_KEY) {
    const ok = loadVapidFromFile();
    if (!ok) return res.json({ publicKey: '' });
  }
...
app.delete('/api/admin/users/:userId', authenticateToken, requireAdmin, (req, res) => {
  const { userId } = req.params;
  if (parseInt(userId, 10) === req.user.id) return res.status(400).json({ error: 'Cannot delete your own account' });
...
app.post('/api/admin/bulk-reset-passwords', authenticateToken, requireAdmin, (req, res) => {
  db.all('SELECT id, username FROM users WHERE id != ?', [req.user.id], (err, users) => {
    ...
    const resetUsers = [];
    let completed = 0;
    if (!users || users.length === 0) return res.json({ resetUsers: [] });
...
app.get('/api/admin/export', authenticateToken, requireAdmin, (req, res) => {
  const exportData = { users: [], messageStats: [], fileStats: [], exportDate: new Date().toISOString() };
  db.all('SELECT * FROM users', (err, users) => {
    ...
    db.all('SELECT COUNT(*) as count FROM messages GROUP BY sender', (err, messageStats) => {
      ...
      db.all('SELECT uploaded_by, COUNT(*) as count, SUM(file_size) as totalSize FROM file_uploads GROUP BY uploaded_by', (err, fileStats) => {
  ...
  socket.on('join_user_room', async (payload) => {
    try {
      ...
      if (payload && typeof payload === 'object') {
        ...
        userId = parseInt(payload, 10);
        token = socket.handshake && socket.handshake.auth ? socket.handshake.auth.token : null;
      }

      if (isNaN(userId)) {
        socket.emit('auth_error', { error: 'Invalid user id for join' });
        return;
      }
      ...
      if (token) {
        ...
        db.get('SELECT id FROM users WHERE id = ?', [userId], (err, row) => {
          ...
          socket.emit('join_success', { userId });
        });
      } else {
        // legacy behavior (no token) - join but no strong auth verification
        socket.join(`user_${userId}`);
        connectedUsers.set(socket.id, userId);
        db.run('UPDATE users SET is_online = 1 WHERE id = ?', [userId], () => {});
        socket.broadcast.emit('user_status_changed', { userId, isOnline: true });
        socket.emit('join_success', { userId, legacy: true });
      }
    } catch (e) {
      console.warn('Error in join_user_room:', e);
      socket.emit('auth_error', { error: 'Internal server error while joining' });
    }
  });
